    ...
    try {
      ...
      channel.messages.fetch(id).then(msg=>{
        if(msg.content && !msg.embeds[0]){
          ...
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<:no:833101993668771842> There is nothing to update`)
        );
        }
        if(msg.embeds[0]){
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes:833101995723194437> Updated the Embed in:`)
...
/**
 * @INFO
 * Bot Coded by Ÿακι ジ#4635 | https://github.com/Tomato6966/Discord-Js-Handler-Template
 * @INFO
